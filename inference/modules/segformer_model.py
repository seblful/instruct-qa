...
class SegformerForMaskDecodeHead(SegformerDecodeHead):
    def __init__(self, config):
        ...
        decoder_layer_hidden_size = getattr(
            config, "decoder_layer_hidden_size", config.decoder_hidden_size)

        # linear layers which will unify the channel dimension of each of the encoder blocks to the same config.decoder_hidden_size
        mlps = []
        for i in range(config.num_encoder_blocks):
            mlp = SegformerForMaskMLP(
                config, input_dim=config.hidden_sizes[i], output_dim=decoder_layer_hidden_size)
            mlps.append(mlp)
        ...
        for encoder_hidden_state, mlp in zip(encoder_hidden_states, self.linear_c):
            if self.config.reshape_last_stage is False and encoder_hidden_state.ndim == 3:
                ...
                encoder_hidden_state = (
                    encoder_hidden_state.reshape(
                        batch_size, height, width, -1).permute(0, 3, 1, 2).contiguous()
                )
